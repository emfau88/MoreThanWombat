...
**Current update — 2026-09-09:** [Plan 31](31_GAMEPLAY_AND_WAVE_COMPLETION_PLAN.md) supersedes the expansion order and exclusions below. The current stage has three zones with distinct backgrounds and seven role-based encounters (2/3/2), a tested visible-floor band, four same-zone sub-wave transitions and two explicit safe travel corridors with arrival triggers. Encounter groups favor several weak Pigeons with selected Flanker, Zoner and Heavy peaks; rewards restore health. Furnace Yard now adds a periodic steam danger, deterministic HP-/MP-lunchbox and the standalone Acting Foreman midboss with a command-triggered steam backfire. Authored transition cards preview each section, while active combat camera framing follows the player/enemy group. The old single-background data example is historical; `src/game/data/stages.ts` is authoritative. G1–G6 are technically implemented; G7 replaces Encounter 7 with the two-phase boss. [Current G6 evidence](qa/g6-runtime-2026-09-09/README.md).
