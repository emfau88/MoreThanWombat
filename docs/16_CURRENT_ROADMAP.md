...
## Current execution update — 2026-09-10
...
- The first pre-G8 UI artwork pass is not accepted as a complete UI. R0 locks a compact two-row action hierarchy and modular HUD semantics; R1 provides adaptive input geometry and a bounded floating joystick. R2 retains the successful ImageGen action-button family. Its generated flat joystick pair was technically valid but visually regressed readability, so the clearer original base/knob artwork is used at the new compact size and with improved idle contrast. R3 replaces the cropped monolithic frames with a complete mirrorable chassis and exact code-owned portrait/HP/MP slots. R4 supplies seven portraits, local Wave HP feedback and centered priority HUD semantics; R5 closes transitions and full-device verification. See the [layout contract](32_BATTLE_UI_R0_LAYOUT_CONTRACT.md) and [UI-R4 report](qa/ui-r4-2026-09-11/README.md).
