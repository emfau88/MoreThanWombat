...
- Typecheck, **99/99 tests** and build pass locally; G0 records six character-sheet hard gates and all three VFX-QA manifests passing.
...
- A pre-G8 UI polish step replaces the temporary vector touch circles with an ImageGen-authored industrial control set, moves Defend into the right-thumb fan, keeps Attack largest and promotes Jump to second-largest. Player, enemy and Boss HUD frames are also pulled forward from G9. The 2026-09-10 correction fills their portrait sockets, contains HP/MP bars within the bevels, enlarges the joystick knob and replaces raw background seams with a transparent industrial transition prop; the remaining G9 threat, camera and audio work stays open. See the [UI report](qa/ui-controls-2026-09-09/README.md).
